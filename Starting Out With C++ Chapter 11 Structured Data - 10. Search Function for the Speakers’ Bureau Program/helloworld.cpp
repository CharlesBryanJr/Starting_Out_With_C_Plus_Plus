/*
Starting Out With C++ Chapter 11 Structured Data - 10. Search Function for the Speakers’ Bureau Program
...
void getCustomerAccount(CustomerAccount customers[])
{
    ...
    for(unsigned int ele{0}; ele<10; ++ele)
    {
        ...
        customers[ele].telephoneNumber = "817-909-3857";
...
void getSpeakerData(Speaker speakerBureau[])
{
    ...
    int speakerToUpdate,
        valueToUpdate;
    std::string serachSpeakers;
    bool matchFound{false};

    speakerBureau[0].speakingTopic = "Jedi";

    while( tolower(makeUpdate == 'y') )
    {
        std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
        std::cout << "Input a speakingTopic you like to serach for : ";
            std::getline(std::cin,serachSpeakers);
        
        for(unsigned int index{0}; index<14; ++index)
        {
            if( serachSpeakers == speakerBureau[index].speakingTopic )
            {
                std::cout << "Key Word Found!" << "\n";
                matchFound = true;
                std::cout << " " << "\n";

                std::cout << "For speaker, " << index+1 << ", here are the details." << "\n";
                    std::cout << "Name : " << speakerBureau[index].name << "\n";
                    std::cout << "telephoneNumber : " << speakerBureau[index].telephoneNumber << "\n";
                    std::cout << "speakingTopic : " << speakerBureau[index].speakingTopic << "\n";
                    std::cout << "feeRequired : " << speakerBureau[index].feeRequired << "\n";
                    std::cout << " " << "\n";
            }
        }
