...
//*****************************************************
// Definition of the ParkingTicket class's accessor functions. 
// These functions return and display the values in these member variables.
//*****************************************************

void ParkingTicket::reportIllegallyParkedCar( const ParkedCar &pCar ) const
{
    std::cout << " --- reportIllegallyParkedCar --- " << "\n"; std::cout << " " << "\n";
    std::cout << pCar.getCarMake() << "\n";
    std::cout << pCar.getCarModel() << "\n";
    std::cout << pCar.getCarColor() << "\n";
    std::cout << " " << "\n";
}

void ParkingTicket::reportFineAmount( const ParkedCar &pCar, const ParkingMeter &pMeter )
{
    std::cout << " --- reportFineAmount --- " << "\n"; std::cout << " " << "\n";
    updateFineAmount(pCar,pMeter);
    std::cout << "FineAmount : " << fineAmount << "\n";
    std::cout << " " << "\n";
}

void ParkingTicket::updateFineAmount( const ParkedCar &pCar, const ParkingMeter &pMeter )
{
    if( pCar.getMinutesParked() > pMeter.getPurchasedParkingTime() )
    {
        fineAmount += 25; // $25 for the first hour or part of an hour that the car is illegally parked

        for( unsigned int i{1}; i<999999; ++i )
        {
            if( i % 60 == 0 )
            {
                if( ( pCar.getMinutesParked() - pMeter.getPurchasedParkingTime() ) > i )
                {
                    fineAmount += 10; // plus $10 for every additional hour or part ofan hour that the car is illegally parked
                }
            }
        }
    }
...
void ParkingTicket::reportPoliceOfficer( const PoliceOfficer &pOfficer ) const
{
    std::cout << " --- reportPoliceOfficer --- " << "\n"; std::cout << " " << "\n";

    std::cout << "Police Officer " << pOfficer.getPoliceOfficerName() << ", " << pOfficer.getPoliceOfficerBadgeNumber() << ", is issuing the ticket." << "\n";
}

//*****************************************************
// Definition of the PoliceOfficer class's mutator functions. 
// These functions return and display the values in these member variables.
// Update other member variables if needed.	
//*****************************************************

void PoliceOfficer::setPoliceOfficer()
{
    std::cout << " --- setPoliceOfficer --- " << "\n"; std::cout << " " << "\n";

    std::cout << "Input the name of the Police Officer : ";
        std::cin >> policeOfficerName;
        std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
        std::cout << " " << "\n";
    
    std::cout << "Input the Badge Number of the Police Officer : ";
        std::cin >> policeOfficerBadgeNumber;
        std::cout << " " << "\n";
}

void PoliceOfficer::examineParkedCar( const ParkedCar &pCar, const ParkingMeter &pMeter, ParkingTicket &pTicket )
{
    std::cout << " --- examineParkedCar --- " << "\n"; std::cout << " " << "\n";

    char newPoliceOfficer{'n'};

    std::cout << "Input a 'y' if we have a new officer : ";
        std::cin >> newPoliceOfficer;
    
        if( newPoliceOfficer == 'y')
        {
            std::cout << " " << "\n";
            setPoliceOfficer();
        }
    
    if( pCar.getMinutesParked() > pMeter.getPurchasedParkingTime() ) // issue ticket
    {
        issueParkedCarTicket( pCar, pMeter, pTicket );
    }

    else // pMeter.getPurchasedParkingTime() >= pCar.getMinutesParked() 
    {
        std::cout << "No Ticket!" << "\n";
    }
...
void PoliceOfficer::issueParkedCarTicket( const ParkedCar &pCar, const ParkingMeter &pMeter, ParkingTicket &pTicket )
{
    std::cout << " --- issueParkedCarTicket --- " << "\n"; std::cout << " " << "\n";
    std::cout << "Police Officer " << policeOfficerName << ", " << policeOfficerBadgeNumber << ", is issuing you a ticket." << "\n";
        std::cout << " " << "\n";
    pTicket.reportIllegallyParkedCar(pCar);
    pTicket.reportFineAmount( pCar, pMeter );
}

//*****************************************************
// Definition of DivSales Class's costructors. 
//*****************************************************

DivSales::DivSales( double &qs1, double &qs2, double &qs3, double &qs4 ) 
{
    while( qs1 < 0 )
    {
        std::cout << "Input Validation: Only accept positive values for quarterly sales figures." << '\n';
        std::cout <<  "Input a quarterly sales figure for Q1: ";
            std::cin >> qs1;
        std::cout << " " << '\n';
    }
    quarterlySales[0] = qs1;

    while( qs2 < 0 )
    {
        std::cout << "Input Validation: Only accept positive values for quarterly sales figures." << '\n';
        std::cout <<  "Input a quarterly sales figure for Q2: ";
            std::cin >> qs2;
        std::cout << " " << '\n';
    }
    quarterlySales[1] = qs2;

    while( qs3 < 0 )
    {
        std::cout << "Input Validation: Only accept positive values for quarterly sales figures." << '\n';
        std::cout <<  "Input a quarterly sales figure for Q3: ";
            std::cin >> qs3;
        std::cout << " " << '\n';
    }
    quarterlySales[2] = qs3;

    while( qs4 < 0 )
    {
        std::cout << "Input Validation: Only accept positive values for quarterly sales figures." << '\n';
        std::cout <<  "Input a quarterly sales figure for Q4: ";
            std::cin >> qs4;
        std::cout << " " << '\n';
    }
    quarterlySales[3] = qs4; 
}
//*****************************************************
// Definition of the DivSales class's mutator functions. 
// These functions return and display the values in these member variables.
// Update other member variables if needed.	
//*****************************************************

void DivSales::setQuarterlySales( double &qs1, double &qs2, double &qs3, double &qs4 )
{
    subtractTotalCorporateSales();
    ...
    addTotalCorporateSales();
}

void DivSales::subtractTotalCorporateSales()
{
    for(unsigned i{0}; i<4; ++i)
    {
        totalCorporateSales -= quarterlySales[i];
    }
}

void DivSales::addTotalCorporateSales()
{
    for(unsigned i{0}; i<4; ++i)
    {
        totalCorporateSales += quarterlySales[i];
...
void displayProgramMenu()
{
    ...
    std::cout << "User, choice a program to run." << "\n";
        ...
        std::cout << "K) Chapter 14 More About Classes - 10. Corporate Sales " << "\n";
        std::cout << "L) " << "\n";
        std::cout << "M)  " << "\n";
        std::cout << "N)  " << "\n";
        std::cout << "O) Chapter 14 More About Classes - 14. Parking Ticket Simulator" << "\n";
        std::cout << "P) Chapter 14 More About Classes - 15. Car Instrument Simulator " << "\n";
        std::cout << "Q)  " << "\n";
        std::cout << "R)  " << "\n";
        std::cout << "S)  " << "\n";
        std::cout << "T)  " << "\n";
